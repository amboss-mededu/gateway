...
- The user service defines a mutation called `loginUser` that [checks if
  the credentials are valid](https://github.com/amboss-mededu/gateway/blob/master/examples/auth/users.go#L66) and responds with a token.
...
- When the gateway receives a query, it [looks for the token](https://github.com/amboss-mededu/gateway/blob/master/examples/auth/gateway.go#L15-L29) and if its present,
  sends the value as the `USER_ID` header when sending queries to the services.

- The other services [uses the header value](https://github.com/amboss-mededu/gateway/blob/master/examples/auth/todo.go#L89) to perform whatever user-specific logic is
  required.

- The current user can query for their User record with the [viewer gateway field](https://github.com/amboss-mededu/gateway/blob/master/examples/auth/gateway.go#L50-L58)
